...
public class ModuleDefSensor extends AbstractMagikFamilySensor<ModuleDefFile> {
  ...
  protected List<Class<? extends Check>> checkClasses() {
    return ModuleDefCheckList.INSTANCE.getBaseChecks();
  }

  @Override
  protected FilePredicate extraPredicate(final FilePredicates predicates) {
    return predicates.hasFilename(ModuleDefSensor.MODULE_DEF);
  }

  @Override
  protected ModuleDefFile parseFile(final URI uri, final String content) {
    final IDefinitionKeeper definitionKeeper = new DefinitionKeeper(false);
    return new ModuleDefFile(uri, content, definitionKeeper, null);
  }

  @Override
  protected void saveMetrics(
      final SensorContext context, final InputFile inputFile, final ModuleDefFile file) {
  ...
  protected void saveCpdTokens(
      final CpdTokenSaver cpdTokenSaver, final InputFile inputFile, final ModuleDefFile file) {
